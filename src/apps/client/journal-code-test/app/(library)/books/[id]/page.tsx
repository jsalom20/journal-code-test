...
import { getBook, getBorrowers } from "@/lib/api"
...
export default async function BookDetailPage({
  ...
  try {
    const [book, borrowers] = await Promise.all([
      getBook(id),
        ...
        <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="mt-5 space-y-4">
              {book.copies.map((copy) => (
                <div key={copy.id} className="rounded-[1.75rem] border border-stone-200 bg-stone-50/70 p-4">
                  ...
                  {copy.circulationStatus === "Available" || copy.circulationStatus === "Reserved" ? (
                    <div className="mt-4">
                      <CheckoutForm copyId={copy.id} borrowers={borrowers} />
                    </div>
                  ) : null}
                </div>
              ))}
          ...
          <div className="space-y-6">
            <SectionCard>
              ...
              <div className="mt-5">
                <ReservationForm bookId={book.id} borrowers={borrowers} />
              </div>
            </SectionCard>
